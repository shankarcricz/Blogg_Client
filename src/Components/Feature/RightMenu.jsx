...
export default function RightMenu() {
  ...
  const {myBlogs} = useSelector(state=>state?.blogs)
    let v, c, cm, t = 0;
  const rows = []
  for(let obj of myBlogs) {
    let temp = {}
    temp.name = obj.title
    temp.calories = obj.views
    v = v + parseInt(obj.views) || 0
    temp.fat = obj.claps
    c = c +  parseInt(obj.claps) || 0
    temp.carbs =  obj.comments.length
    cm = cm +  parseInt(obj.comments.length) || 0
    temp.protein = 5
    t = t + 5
    rows.push(temp)
  }
